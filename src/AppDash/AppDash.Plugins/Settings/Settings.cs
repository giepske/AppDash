...
{
    public abstract class Settings<TPlugin, TRazorComponent> : IDisposable, ISettings where TPlugin : AppDashPlugin where TRazorComponent : SettingsComponent
    {
        ...
        protected Settings(TPlugin plugin)
        {
            Plugin = plugin;
        ...
        private void Initialize(IRepository<PluginSettings> pluginSettingsRepository)
        {
            ...
            SettingsData = new PluginData
            {
                Data = settings?.Data != null ?
                    JsonConvert.DeserializeObject<Dictionary<string, Tuple<Type, object>>>(settings.Data) :
            ...
            if (settings == null)
            {
                pluginSettingsRepository.Insert(new PluginSettings
                {
                    ...
                    Data = JsonConvert.SerializeObject(new Dictionary<string, Tuple<Type, object>>())
                });
            }
        }
